...
일반적으로 어떠한 값을 문 형태의 코드로 정의하는 방법은 리터럴이라고도 부릅니다.\
...
### 문(Statement)을 이용한 생성 vs 식(Expression)을 이용한 생성


이 둘의 가장 큰 차이점은 정의/호출가능 시점의 차이가 있는데요.\
[호이스팅](/javascript/호이스팅)이라는 매커니즘에 의해 발생하는 현상입니다.

자세한 내용은 별도의 포스팅으로 다뤄보겠습니다.\
지금은 그냥 **함수선언문으로 만들어진 함수는 코드 위치에 상관없이 어디서든 호출이 가능하다!**로 이해하면 될 것 같습니다!
